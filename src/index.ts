  ...
  const crypto = {
    asymmetric: {
      ...
      keyPair: (
        ...
        const keypair = seed
          ? sodium.crypto_box_seed_keypair(crypto.stretch(seed))
      ...
      encrypt: ({ secret, recipientPublicKey, senderSecretKey }: EncryptParams): string => {
        ...
        if (senderSecretKey === undefined) {
          ...
          const senderKeys = crypto.asymmetric.keyPair()
    ...
    signature: {
      ...
      keyPair: (
        ...
        const keypair = seed
          ? sodium.crypto_sign_seed_keypair(crypto.stretch(seed))
